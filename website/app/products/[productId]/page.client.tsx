...
export default function ProductDetailPage({ params }: { params: { productId: string } }) {
  ...
  useEffect(() => {
    ...
    setReviews(getMockReviews(params.productId))
  }, [params.productId])
      ...
      <div className="container mx-auto px-4 py-12">
        <div className="grid lg:grid-cols-2 gap-12">
          ...
          <div className="space-y-4">
            <div className="relative aspect-square rounded-2xl overflow-hidden bg-black/40 border border-white/10">
              ...
              <div className="absolute top-6 left-6 px-4 py-2 rounded-full bg-white/10 backdrop-blur-sm border border-white/20">
                <span className="text-white/90 text-sm font-light">{product.category}</span>
              </div>
            </div>
              ...
              {product.images.map((image, index) => (
                <button
                  ...
                  className={`relative w-24 h-24 rounded-lg overflow-hidden border-2 transition-all ${
                    index === currentImageIndex ? "border-white" : "border-white/20 hover:border-white/40"
                  }`}
                >
                  <img
                    src={image || "/placeholder.svg"}
          ...
          <div className="space-y-6">
            <div>
              <h1 className="text-4xl font-light text-white mb-4 text-balance">{product.name}</h1>
              <p className="text-lg text-white/70 leading-relaxed">{product.description}</p>
            </div>
            ...
            <div className="flex items-center gap-4">
              <div className="flex items-center gap-1">
                {[...Array(5)].map((_, i) => (
                  <Star
                    key={i}
                    className={`w-5 h-5 ${i < Math.round(averageRating) ? "fill-yellow-400 text-yellow-400" : "text-white/20"}`}
                  />
                ))}
              </div>
            ...
            <div className="bg-white/5 backdrop-blur-sm rounded-2xl p-6 border border-white/10">
              ...
              <ul className="space-y-3 text-white/70">
                <li className="flex items-start gap-3">
                  ...
                  <span>Lifetime access to all files and updates</span>
                </li>
                <li className="flex items-start gap-3">
                  <span className="text-white mt-1">✓</span>
        ...
        {/* Reviews Section */}
        <div className="mt-16">
          <h2 className="text-3xl font-light text-white mb-8">Customer Reviews</h2>
          <div className="grid gap-6">
            {reviews.map((review) => (
              <div key={review.id} className="bg-white/5 backdrop-blur-sm rounded-2xl p-6 border border-white/10">
                <div className="flex items-start justify-between mb-4">
                  <div>
                    <h4 className="text-lg text-white font-normal">{review.username}</h4>
